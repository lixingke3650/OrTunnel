            ...
            if (globals.G_SECRET_FLAG == True and globals.G_SECRET_TYPE == 'ARC4'):
                while True:
                    ...
                    if not buffer:
                        ...
                        break
                    tunnelworker._Server_Client_Socket.sendto(tunnelworker._Crypt.enCrypt(buffer), tunnelworker._FromClientAddr)
            else :
                while True:
                    buffer = tunnelworker._Server_App_Socket.recv(globals.G_SOCKET_RECV_MAXSIZE)
                    if not buffer:
                        globals.G_Log.info('server socket close. [PostService.py:PostService:stocrun]')
                        break
                    tunnelworker._Server_Client_Socket.sendto(buffer, tunnelworker._FromClientAddr)
        except AttributeError as e:
            # socket被关闭后无法读写，输出DEBUG日志
